...
ImageWidget::ImageWidget(QPixmap* pixmap, const cv::Mat& img_ori)
{
    m_pix = *pixmap;
...
void ImageWidget::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
    if (event->button() == Qt::LeftButton)
    {
        ...
        //double z = img_ori.at<double>(m_startPos.y(), m_startPos.x());
        //std::cout << m_startPos.x() << "  " << m_startPos.y() << "  " << z << std::endl;

        //QPointF point = (event->pos() - m_startPos) * m_scaleValue;
        //std::cout << " "<< point.x() << "  " << point.y() << "  " << z << std::endl;
